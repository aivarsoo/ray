...
class PendulumSwingUpParams:
    ...
    reward_offset = np.ceil(theta_penalty * np.pi ** 2 + theta_dot_penalty *
                            max_speed ** 2 + action_penalty * max_torque ** 2)
    ...
    def __str__(self):
        ...
        _reward_params = {'theta_penalty': self.theta_penalty,
                          'theta_dot_penalty': self.theta_dot_penalty, 'action_penalty': self.action_penalty}
...
class PendulumEnv(Env):
    ...
    def do_simulation(self, u: np.ndarray):
        ...
        newthdot = thdot + (-3 * self.params.g / (2 * self.params.l) *
                            np.sin(th + np.pi) + 3. / (self.params.m * self.params.l ** 2) * u) * dt
        newth = th + newthdot * dt
        newthdot = np.clip(newthdot, -self.params.max_speed,
                           self.params.max_speed)  # pylint: disable=E1111
    ...
    def reset(self, seed: int = None, options: Dict = {}) -> np.ndarray:
        self.t = 0
        if self._mode == "train":
    ...
    def _reward_fn(self, states: Array, actions: Array, next_states: Array, is_tensor: bool = True) -> Array:
        ...
        if is_tensor:
            ...
            action_squared = actions.clamp(-self.params.max_torque,
                                           self.params.max_torque)
            costs = self.params.theta_penalty * th_norm ** 2 + self.params.theta_dot_penalty * \
                thdot ** 2 + self.params.action_penalty * action_squared.squeeze() ** 2
...
class SafePendulumEnv(SafeEnv, PendulumEnv):
    ...
    def _safety_cost_fn(self, states: np.ndarray, actions: np.ndarray, next_states: np.ndarray) -> np.ndarray:
        ...
        unsafe_angle_middle = 0.5 * \
            (self.params.unsafe_max + self.params.unsafe_min)  # 25 = (20 + 30) /2
        ...
        unsafe_mask = np.float64(self._is_near_unsafe_area_batch(
            thetas))  # 20-45 = -25 <= theta <= 75 = 30+45
