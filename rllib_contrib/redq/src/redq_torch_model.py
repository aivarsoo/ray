...
class REDQTorchModel_old(TorchModelV2, nn.Module):
    """Extension of the standard TorchModelV2 for REDQ.
    ...
    def __init__(
        ...
        """Initializes a REDQTorchModel instance.
                ...
                Args:
                    ...
                    ensemble_size (int): The number of models in the ensemble of
                        models (default: 2).
        ...
        super(REDQTorchModel, self).__init__(
        ...
        # Build the Q-network(s). Instead of twin_q flag we simply build a list of q_nets
        self.q_net_list = nn.ModuleList([])
        for idx in range(ensemble_size):
            self.q_net_list.add_module(f"Q_{idx}",
                                       self.build_q_model(
                                           self.obs_space, self.action_space, q_outs, q_model_config, f"q_{idx}"
                                       )
                                       )
    ...
    def build_policy_model(self, obs_space, num_outputs, policy_model_config, name):
        """Builds the policy model used by this REDQ.

        Override this method in a sub-class of REDQTFModel to implement your
        own policy net. Alternatively, simply set `custom_model` within the
        top level REDQ `policy_model` config key to make this default
    ...
    def build_q_model(self, obs_space, action_space, num_outputs, q_model_config, name):
        """Builds one of the (list of) Q-nets used by this REDQ.

        Override this method in a sub-class of REDQTFModel to implement your
        own Q-nets. Alternatively, simply set `custom_model` within the
        top level REDQ `q_model_config` config key to make this default implementation
    ...
    def get_q_values(
        ...
        for idx in critic_idx:
            cur_res = self._get_q_value(
                model_out, actions, self.q_net_list.get_submodule(f'Q_{idx}'))
            # cur_res = self._get_q_value(model_out, actions, self.q_net_list[idx])
            res[0].append(cur_res[0])
            res[1].append(cur_res[1])
        return res[0], res[1]
    ...
    def _get_q_value(self, model_out, actions, net):
        ...
        q_vals, state = net(input_dict, [], None)
        return q_vals, state
